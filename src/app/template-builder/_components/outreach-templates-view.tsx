...
import { OutreachTemplateDialog } from "./outreach-template-dialog";
import { OutreachTemplateCard } from "./outreach-template-card";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function OutreachTemplatesView() {
  ...
  const handleSaveTemplate = (templateData: Omit<OutreachTemplate, 'id'> & {id?: string}) => {
    if (templateData.id) {
      setTemplates(templates.map(t => (t.id === templateData.id ? { ...t, ...templateData } as OutreachTemplate : t)));
    } else {
      const newTemplate: OutreachTemplate = {
        ...templateData,
        id: `outreach-template-${Date.now()}`
      };
    ...
    <div className="space-y-4">
       <div className="flex items-center justify-between">
            <div className="flex-1">
                 <h2 className="text-2xl font-semibold tracking-tight">Outreach Templates</h2>
                 <p className="text-muted-foreground">
                    Create and manage reusable templates for your sales and outreach messages.
                </p>
            </div>
      ...
      {templates.length === 0 ? (
         <div className="flex flex-col items-center justify-center text-center py-24 border-2 border-dashed rounded-lg">
          ...
          <p className="text-muted-foreground mt-2 mb-4">Click "New Template" to create your first outreach template.</p>
          <Button onClick={handleAddTemplate}>
            <PlusCircle className="mr-2"/> New Template
          </Button>
        </div>
